  ...
  return (
    <main style={{
      ...
      <div style={{
        ...
        top: '-15%', left: '-10%', pointerEvents: 'none', animation: 'float 8s ease-in-out infinite'
      }} />
      <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 32 }}>
          ...
          <span style={{ fontSize: 18, fontWeight: 800, color: '#fff', letterSpacing: -0.5 }}>Vanta</span>
        </div>

        {/* Progress bar — only during signup */}
        {mode === 'signup' && (
              ...
              {STEPS.map((s, i) => (
                <div key={s} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <div style={{
                    ...
                    background: step > i + 1 ? 'linear-gradient(135deg, #6366f1, #06b6d4)' : step === i + 1 ? 'rgba(99,102,241,0.3)' : 'rgba(255,255,255,0.05)',
                    border: step === i + 1 ? '2px solid #6366f1' : '2px solid transparent',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 11, color: step > i + 1 ? '#fff' : step === i + 1 ? '#6366f1' : '#444',
        ...
        {mode === 'login' && (
          <div>
            ...
            <button onClick={handleLogin} disabled={loading} style={{
              ...
              cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 20px rgba(99,102,241,0.3)'
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{
                ...
                fontSize: 14, color: '#6366f1', fontWeight: 700, zIndex: 1
              }}>🇰🇪 +254</div>
            ...
            <button onClick={handlePhoneStep} style={{
              width: '100%', padding: '15px', background: 'linear-gradient(135deg, #6366f1, #06b6d4)',
              color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 700,
              cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 20px rgba(99,102,241,0.3)'
            }}>Continue →</button>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              {typeCards.map(({ id, icon, label, desc }) => (
                <div key={id} onClick={() => setAccountType(id)} style={{
                  padding: '20px 16px', borderRadius: 14, cursor: 'pointer',
                  border: `1.5px solid ${accountType === id ? '#6366f1' : 'rgba(255,255,255,0.08)'}`,
                  background: accountType === id ? 'rgba(99,102,241,0.12)' : 'rgba(255,255,255,0.02)',
                  transition: 'all 0.2s'
                }}
            ...
            <button onClick={handleTypeStep} style={{
              width: '100%', padding: '15px', background: accountType ? 'linear-gradient(135deg, #6366f1, #06b6d4)' : 'rgba(255,255,255,0.05)',
              color: accountType ? '#fff' : '#444', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 700,
              cursor: accountType ? 'pointer' : 'not-allowed', fontFamily: 'inherit',
              boxShadow: accountType ? '0 4px 20px rgba(99,102,241,0.3)' : 'none', transition: 'all 0.3s'
            }}>{ loading ? 'Saving...' : accountType === 'business' ? 'Add business details →' : 'Finish setup →' }</button>
            {message && <div style={{ marginTop: 12, padding: '12px 16px', background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.3)', borderRadius: 10, fontSize: 13, color: '#f87171' }}>{message}</div>}
          </div>
        )}
        ...
        {mode === 'signup' && step === 4 && (
          <div>
            ...
            <button onClick={saveProfile} disabled={loading} style={{
              ...
              color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 700,
              cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 20px rgba(99,102,241,0.3)'
            }}>{ loading ? 'Saving...' : 'Complete registration ✓' }</button>
            {message && <div style={{ marginTop: 12, padding: '12px 16px', background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.3)', borderRadius: 10, fontSize: 13, color: '#f87171' }}>{message}</div>}
          </div>
        )}
